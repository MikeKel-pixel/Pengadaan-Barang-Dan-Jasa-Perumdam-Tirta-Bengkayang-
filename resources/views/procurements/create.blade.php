...
<head>
    ...
    <title>Buat Pengajuan - Perumdam</title>
...
<body class="bg-gray-100">
    <nav class="bg-green-600 text-white shadow-lg">
        <div class="container mx-auto px-6 py-3">
            <div class="flex justify-between items-center">
                <div>
                    <a href="{{ route('pengadaan.dashboard') }}" class="text-xl font-bold">PERUMDAM - Buat Pengajuan</a>
                </div>
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Buat Pengajuan Baru</h1>
            ...
            <form action="{{ route('procurements.store') }}" method="POST" id="procurementForm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="harga_estimasi[]" class="harga-input w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500" 
                                           placeholder="Harga" step="1000" required>
                                </div>
                            </div>
                            <div class="mt-2 text-right">
                                <span class="text-sm text-gray-500">Subtotal: Rp <span class="subtotal-display">0</span></span>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3" 
                              class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-bold text-2xl text-blue-600">Rp <span id="grandTotal">0</span></span>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateGrandTotal() {
            ...
            document.querySelectorAll('.subtotal-display').forEach(display => {
                total += parseInt(display.innerText.replace(/\./g, '')) || 0;
            });
        ...
        function setHargaDefault(select, row) {
            ...
            if (hargaDefault && hargaDefault > 0) {
                ...
                // Tampilkan satuan
                const satuanText = document.createElement('span');
                satuanText.className = 'text-xs text-gray-500 ml-2';
                satuanText.innerText = `Satuan: ${satuan}`;
                
                const parent = select.parentElement;
                const existing = parent.querySelector('.satuan-info');
                if (existing) existing.remove();
                parent.appendChild(satuanText);
            }
        ...
        document.getElementById('addItemBtn').addEventListener('click', function() {
            ...
            // Hapus satuan info
            const satuanInfo = newRow.querySelector('.satuan-info');
            if (satuanInfo) satuanInfo.remove();
            
            // Tambah event listener
            const select = newRow.querySelector('.item-select');
            select.addEventListener('change', function() {
                setHargaDefault(this, newRow);
            });
