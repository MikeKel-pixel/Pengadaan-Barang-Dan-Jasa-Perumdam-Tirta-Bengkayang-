...
<head>
    ...
    <title>Edit Barang - Perumdam</title>
    ...
    <nav class="bg-blue-600 text-white shadow-lg">
        <div class="container mx-auto px-6 py-3">
            <div class="flex justify-between items-center">
                <div>
                    <a href="{{ route('admin.dashboard') }}" class="text-xl font-bold">PERUMDAM - Edit Barang</a>
                </div>
            ...
            <form action="{{ route('items.update', $item->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="satuan" id="satuan" 
                            ...
                            <option value="unit" {{ old('satuan', $item->satuan) == 'unit' ? 'selected' : '' }}>Unit</option>
                            <option value="meter" {{ old('satuan', $item->satuan) == 'meter' ? 'selected' : '' }}>Meter</option>
                            <option value="kg" {{ old('satuan', $item->satuan) == 'kg' ? 'selected' : '' }}>Kg</option>
                            <option value="liter" {{ old('satuan', $item->satuan) == 'liter' ? 'selected' : '' }}>Liter</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="harga_estimasi_default" id="harga_estimasi_default" 
                               class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500"
                               value="{{ old('harga_estimasi_default', $item->harga_estimasi_default) }}" step="1000">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="spesifikasi" id="spesifikasi" rows="4" 
                              class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">{{ old('spesifikasi', $item->spesifikasi) }}</textarea>
                    @error('spesifikasi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
