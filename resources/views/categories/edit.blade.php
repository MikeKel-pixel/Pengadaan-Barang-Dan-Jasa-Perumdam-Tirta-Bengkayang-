...
<head>
    ...
    <title>Edit Kategori - Perumdam</title>
    ...
    <nav class="bg-blue-600 text-white shadow-lg">
        <div class="container mx-auto px-6 py-3">
            <div class="flex justify-between items-center">
                <div>
                    <a href="{{ route('admin.dashboard') }}" class="text-xl font-bold">PERUMDAM - Edit Kategori</a>
                </div>
            ...
            <form action="{{ route('categories.update', $category->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" 
                              class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">{{ old('deskripsi', $category->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
